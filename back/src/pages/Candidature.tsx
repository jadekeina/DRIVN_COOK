...
const Candidatures: React.FC = () => {
    ...
    const getStatusColor = (statut: string) => {
        switch (statut) {
            ...
            case 'en_cours': return '#FFA9A3';
            case 'acceptee': return '#28a745';
            case 'refusee': return '#F87575';
    ...
    const filteredCandidatures = candidatures.filter(candidature => {
        const matchesStatus = selectedStatus === 'all' || candidature.statut === selectedStatus;
    ...
    const handleStatusChange = async (candidatureId: number, newStatus: string) => {
        try {
    ...
    return (
        <div className="p-6">
            ...
            <div className="mb-8">
                ...
                <p className="text-gray-600">Gérez les demandes de franchise ({candidatures.length} candidature{candidatures.length > 1 ? 's' : ''})</p>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-6">
                    {[
                        ...
                        { label: 'En attente', count: candidatures.filter(c => c.statut === 'en_attente').length, color: '#5C95FF' },
                        { label: 'En cours', count: candidatures.filter(c => c.statut === 'en_cours').length, color: '#FFA9A3' },
                        { label: 'Acceptées', count: candidatures.filter(c => c.statut === 'acceptee').length, color: '#28a745' }
                    ].map((stat, index) => (
                        ...
                        {filteredCandidatures.map((candidature) => (
                            <tr key={candidature.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <div className="text-sm text-gray-500">{candidature.telephone}</div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                                    <div className="text-sm text-gray-900">{candidature.ville}</div>
                                    <div className="text-sm text-gray-500 capitalize">{candidature.zone}</div>
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap">
                    <span
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div className="flex items-center justify-end space-x-2">
                                        ...
                                        {candidature.statut === 'en_attente' && (
                                            <>
                                                <button
                                                    onClick={() => handleStatusChange(candidature.id, 'acceptee')}
                                                ...
                                                <button
                                                    onClick={() => handleStatusChange(candidature.id, 'refusee')}
