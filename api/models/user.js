...
const User = {
    ...
    findById: (id, callback) => {
        db.query("SELECT id, email, first_name, last_name, role, phone, is_verified, created_at FROM users WHERE id = ?", [id], (err, results) => {
            if (err) return callback(err);
            callback(null, results[0]);
        });
    },
    ...
    getAll: (callback) => {
        db.query("SELECT id, email, first_name, last_name, role, phone, is_verified, created_at FROM users ORDER BY created_at DESC", callback);
    },
